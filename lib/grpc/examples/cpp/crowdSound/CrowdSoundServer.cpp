...
class CrowdSoundServer final : public CrowdSound::CrowdSound::Service {
	Status ListSongs(ServerContext* context, const ListSongsRequest* request, ServerWriter<ListSongsResponse>* writer) override {
		ListSongsResponse* response = new ListSongsResponse();
		response->set_isplaying(true);
		response->set_name("Test");

		writer->Write(*response);

		ListSongsResponse* response2 = new ListSongsResponse();
		response2->set_isplaying(false);
		response2->set_name("Test2");
